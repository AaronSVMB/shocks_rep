...
doc = """
Survey with additional questions for the additive ambiguous experiment
"""

class C(BaseConstants):
    NAME_IN_URL = 'survey_additive_ambiguous'
...
class Player(BasePlayer):
    ...
    adjustment_positive = models.IntegerField(
        choices=[
            [1, 'Strongly Agree'],
            [2, 'Agree'],
            [3, 'Disagree'],
        ...
        label="I tended to invest more to the group account after a +5 adjustment in the previous period."
    )

    adjustment_negative = models.IntegerField(
        choices=[
            [1, 'Strongly Agree'],
            [2, 'Agree'],
            [3, 'Disagree'],
        ...
        widget=widgets.RadioSelectHorizontal,
        label="I tended to invest less to the group account after a -5 adjustment in the previous period."
    )

    adjustment_unaffected = models.IntegerField(
        choices=[
            [1, 'Strongly Agree'],
            [2, 'Agree'],
            [3, 'Disagree'],
        ...
        widget=widgets.RadioSelectHorizontal,
        label="My investment decisions were unaffected by the +/- 5 adjustment in the previous period."
    )

    # Ambiguous Specific Qs

    patterns = models.IntegerField(
        choices=[
            [1, 'Yes'],
            [2, 'No']
        ],
        widget=widgets.RadioSelect,
        label="7. Did you notice any patterns in the computer's adjustments?"
    )

    patterns_explained = models.LongStringField(label="If you think you noticed any patterns in the computer's "
                                                      "adjustments, please describe them below. (Type 'No pattern' if you"
    ...
    clarity = models.IntegerField(
        ...
        label='10. The instructions for the experiment were clear and easy to follow',
        widget=widgets.RadioSelectHorizontal
    )

    suggestions = models.LongStringField(label='11. Thank you for completing this experiment. We value your feedback, '
    ...
    time_spent_survey_one_point_five = models.FloatField(initial=0)
    ...
    time_spent_survey_five = models.FloatField(initial=0)
...
class SurveyOnePointFive(Page):
    form_model = 'player'
    form_fields = ['adjustment_positive', 'adjustment_negative', 'adjustment_unaffected']
    ...
    @staticmethod
    def before_next_page(player: Player, timeout_happened):
        player.time_spent_survey_one_point_five = time.time() - player.start_time  # Save invest decision time spent


class SurveyTwo(Page):
    form_model = 'player'
    form_fields = ['patterns', 'patterns_explained']
...
class SurveyFive(Page):
    ...
    @staticmethod
    def before_next_page(player: Player, timeout_happened):
        player.time_spent_survey_five = time.time() - player.start_time  # Save invest decision time spent
