...
doc = """
Standard single group public goods game (linear VCM) played for multiple, finite number of rounds 
"""
...
class Player(BasePlayer):
    ...
    investment = models.FloatField(min=0.00, max=C.ENDOWMENT, decimal_places=2, label="How much will you invest to your group's joint account?")
...
def set_payoffs(group: Group):
    ...
    for p in players:
        p.payoff = (C.ENDOWMENT - p.investment + group.individual_share) / C.RATE
        # Since we are using decimals and oTree points must be Integers I cannot use their points feature
        p.payoff_to_display = C.ENDOWMENT - p.investment + group.individual_share
